...
The implementation is on `dev/forge/forge-project`, retaining the original design files. Targeted project tests cover duplicate launches, payload conflicts, stopped/replaced coordinators, ownership, archive/restore, stale checkpoints, foreign references, receipt/export recovery, filesystem escapes, CLI routing, and partial mesh responses. Separate PostgreSQL tests exercise the complete migration catalog, persistence, contention exceeding pool size, receipts, and compare-and-swap updates in disposable schemas.

Release acceptance also requires native Claude/Codex project sessions on the intended hosts, persisted replay and restart checks, iOS project navigation/launch/rendering tests, and verified TestFlight processing. Record exact results and deployed commits in the release evidence; this contract is not itself a claim that all deployment checks have finished.
